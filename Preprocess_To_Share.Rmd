...
# for isaac to run optional preprocessing
...
# identify system generated texts: common messages across pts
## only seems to appear in android devices except discord commands
#sys.txt = read_csv("EARS_language_dictionaries/text_msg_exclusion.csv")
...
combined = combined %>%
  mutate(strinput_text = ifelse(subjectID %in% android$subject_id, gsub(".*…$","",strinput_text), strinput_text)) %>%
...
combined_copy$toremove<-remove_search_text_entries(combined_copy$text_clean)
...
# function for removing search strings 
```{r, echo=FALSE}
